...
local enemies = {}
local enemyCount = 0
local randomEnemySpawnInterval = 3
SpawnEnemyTimer = 0
...
function SpawnEnemy()
  ...
  e.scoringFixture = love.physics.newFixture(e.body, e.scoringShape)
  e.scoringFixture:setUserData(e)
  e.scoringFixture:setSensor(true)
...
function love.draw()
  ...
  for _, e in ipairs(enemies) do
    if e then
      ...
      love.graphics.polygon("fill", e.body:getWorldPoints(e.scoringShape:getPoints()))
    end
  end
end
...
function beginCollision(a, b, coll)
  ...
  if objA and objB then
    ...
    if objA.name == "player" and objB.name == "enemy" then
      print(tostring(a))
      print(tostring(b))
      print(tostring(objA.scoringFixture))
